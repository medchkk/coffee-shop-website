...
const setupFormHandlers = () => {
  ...
  document.getElementById('registerForm')?.addEventListener('submit', async (e) => {
    ...
    try {
        await handleAuthRequest('http://localhost:5000/api/auth/register', formData);
  ...
  document.getElementById('loginForm')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    try {
        const result = await handleAuthRequest('http://localhost:5000/api/auth/login', getFormData(e.target));
  ...
  document.getElementById('resetPasswordForm')?.addEventListener('submit', async (e) => {
    ...
    try {
        await handleAuthRequest('http://localhost:5000/api/auth/reset-password', formData, true);
        window.location.href = '/frontend/pages/login.html';
    } catch (err) {
        showError(err.message);
    }
